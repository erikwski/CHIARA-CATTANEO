...
var bigpicture = (function () {
  ...
  bpContainer.onmousemove = function (e) {
    ...
    if (movingText) {
      $(movingText).data(
        "x",
        $(movingText).data("x") +
          (e.pageX - previousMousePosition.x) * current.zoom
      );
      $(movingText).data(
        "y",
        $(movingText).data("y") +
          (e.pageY - previousMousePosition.y) * current.zoom
      );
  ...
  function onZoom(zoom, wx, wy, sx, sy) {
    ...
    wx =
      typeof wx === "undefined"
        ? current.x + (window.innerWidth / 2) * current.zoom
        : wx;
    wy =
      typeof wy === "undefined"
        ? current.y + (window.innerHeight / 2) * current.zoom
        : wy;
  ...
  if (navigator.appVersion.indexOf("Mac") != -1) {
    ...
    mousewheel = function (e) {
      ...
      if (!mousewheeltimer) {
        mousewheeltimer = setTimeout(function () {
          onZoom(
            mousewheeldelta > 0 ? current.zoom / 1.7 : current.zoom * 1.7,
    ...
    mousewheel = function (e) {
      ...
      onZoom(
        delta > 0 ? current.zoom / 1.7 : current.zoom * 1.7,
        current.x + e.clientX * current.zoom,
        current.y + e.clientY * current.zoom,
        e.clientX,
        e.clientY
